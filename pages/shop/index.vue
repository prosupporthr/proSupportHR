...
const { fetchProducts: fetch, categories } = useCategory() 
  ...
  fetch() 
  watchEffect(async () => { 
    fetchProducts({price: price.value, category: category?.value === "All" ? "" : category?.value, title: title?.value })
  })
</script>
<template>
  <div class="w-full flex flex-col lg:gap-0 gap-6 ">
    ...
    <div class=" w-full flex justify-center lg:pt-20 ">
      <div class=" lg:max-w-[85%] flex lg:px-0 px-3 w-full gap-6 flex-col lg:gap-10  ">
        ...
        <div :class="loading ? ' flex ' : ' hidden ' " class=" w-full flex justify-center font-semibold " >
          {{ loading ? "Loading..." : "" }}
        </div>
